...
std::vector<std::unique_ptr<Entity>> Collection::get_all_entities() const
{
    return active_segment_->get_all_entities();
}
...
bool Collection::create_index(const IndexMeta & index_meta)
{
    ...
    // 扫描集合，为现有数据构建索引
    std::vector<std::unique_ptr<Entity>> entities = active_segment_->get_all_entities();
