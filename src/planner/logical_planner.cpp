#include "dreamdb/planner/logical_planner.h"
...
std::unique_ptr<LogicalPlanNode> LogicalPlanner::plan(const AstNode & ast) const
{
    // 根据抽象语法树的类型，选择对应的计划
    switch (ast.get_type()) {
...
std::unique_ptr<LogicalPlanNode> LogicalPlanner::plan_select(const SelectStmt & select_stmt) const
{
    (void)select_stmt;
    throw std::runtime_error("Select plan is not implemented");
}

std::unique_ptr<LogicalPlanNode> LogicalPlanner::plan_delete(const DeleteStmt & delete_stmt) const
{
    (void)delete_stmt;
    throw std::runtime_error("Delete plan is not implemented");
}

std::unique_ptr<LogicalPlanNode> LogicalPlanner::plan_update(const UpdateStmt & update_stmt) const
{
    (void)update_stmt;
